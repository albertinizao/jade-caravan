...
- el motor de cálculo ya sigue la especificación `calculation-engine` de OpenSpec en su núcleo: derivados efectivos de carros, consumo efectivo, fuerza de tiro ajustada por mejoras y velocidad basada en la tabla documental, todo con desglose y bloqueadores reproducibles;
...
- el ciclo diario además conserva referencias de regalos de carga/tesoro, aplica la reducción especial de tesoro al superar el último regalo y mantiene el `Caravan` enriquecido con dotes activas para que el cálculo contextual use sus efectos;
- la especificación `initial-state` ya está materializada como seed observada en `backend/src/main/resources/initial-state/observed-campaign.json` y en `backend/src/main/java/com/jadecaravan/application/campaign/initialstate/ObservedInitialStateSeed.java`, preservando 28 carros, 80 viajeros, 63 bestias, 126 de consumo visible y 347 de salario visible, además de exponer advertencias de calidad de datos;
- el dominio ya modela dotes de caravana (`CaravanFeatInstance`) y rasgos de carro para resolver la primera destrucción protegida por `Levantarse de la nada`, además de resolver reparaciones con consumo de material y restauración de `15 × nivel de caravana` PG;
...
- el frontend ya cubre la navegación funcional principal de la campaña: selector de campaña en `/`, tablero en `/campaigns/:campaignId/dashboard`, ciclo diario, carros, viajeros, inventario, bestias, comercio, historial y decisiones de reglas; el `AppShell` ya enlaza estas pantallas de forma contextual y el `campaignStore` recuerda campañas recientes en `localStorage`;
...
- pruebas del estado inicial observado que cubren la importación determinista desde el Excel, la ocupación fraccionaria de `Familias Kalsgard` y la preservación del `Carro taberna` como instancia observada de `Carro vacío`.
